...
out_name = "{:s}/{:d}D/seed{:d}/lr{:s}".format(
    args.out_dir, f, seed, str(l_rate).replace(".", "-")
)
...
if os.path.isfile(drug_list_fname):
    selected_drugs = list(
        pd.read_csv(drug_list_fname, header=None)[0].values.astype(str)
    )
else:
...
print(
    "TRAIN: cell lines =",
    ss_df.shape[0],
    " # after remove empty rows =",
    ss_df.dropna(how="all").shape[0],
    " (for subset of drugs)",
)
print(
    "TEST: cell lines =",
    ss_test_df.shape[0],
    " # after remove empty rows =",
    ss_test_df.dropna(how="all").shape[0],
    " (for subset of drugs)",
)
...
WP = (np.matrix(prng.rand(xdim, f)) - 0.5) / 10.0
prng = np.random.RandomState(seed)
WQ = (np.matrix(prng.rand(ydim, f)) - 0.5) / 10.0
...
def save_and_predict(
    ...
    if suffix == "end":
        ...
        train_pred_df = pd.DataFrame(
            train_pred_mat, columns=ss_df.columns, index=ss_df.index
        )
        ...
        pd.DataFrame(P_mat_train, index=ss_df.index, columns=range(1, f + 1)).to_csv(
            os.path.join(out_name, "CaDRReS_P.csv")
        )
        pd.DataFrame(Q_mat_train, index=ss_df.columns, columns=range(1, f + 1)).to_csv(
            os.path.join(out_name, "CaDRReS_Q.csv")
        )
    ...
    if suffix == "end":
        ...
        new_out_fname = os.path.join(
            out_name, "CaDRReS_pred_{:d}_ckpt.csv".format(epoch)
        )
        new_out_dict_fname = os.path.join(
            out_name, "CaDRReS_pred_{:d}_ckpt.pickle".format(epoch)
        )
...
for epoch in range(1, max_iterations + 1):
    ...
    print(
        "{:d}\t{:.3f}\t({:.3f}%)\t{:.3f}".format(
             epoch, new_err, percent_improved, new_ndcg
        )
    )
